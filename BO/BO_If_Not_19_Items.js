function Start_To_Compare_BO_If_Not_19_Items(namePKG, nameJapanOfPkg, namePhysicTableT_REL, namePhysicTableWK) {
...
function SubStrb_To_Insert_WK_BO_If_Not_19_Items (json_table_WK) {
  let output = `
    LOOP UTL_FILE.GET_LINE(WK_FILE_HANDLE, WK_V_WRITCHAR);
      -- エンドレコードのレコード区分（'END  '）になるまで処理を行う
      EXIT WHEN SUBSTRB(WK_V_WRITCHAR, 1, 5) = STR_END;
...
function Insert_WK_To_Insert_T_REL_BO_If_Not_19_Items(json_table_WK ,namePhysicTableWK) {
  let beforeInsert = `
...
function Insert_T_REL_To_End_BO_If_Not_19_Items(parse_json_mapping, json_table_T_REL, namePhysicTableT_REL, namePhysicTableWK, namePKG) {
